...
- Built an end-to-end ML pipeline on the Wisconsin breast cancer dataset (569 samples), comparing logistic
  regression, random forest, gradient boosting, SVM and XGBoost with stratified 5-fold and nested cross-validation;
...
- Shipped a Streamlit app that returns a prediction and a per-input SHAP explanation, backed by 48 pytest
...
- Designed a leakage-safe evaluation: scaler inside every `Pipeline`, stratified 80/20 split with the test set
  used once, and nested CV (GridSearchCV inside each outer fold) to compare five tuned classifiers without
...
- Engineered for reproducibility: typed, documented `src/` package, 48 pytest tests (including a headless
