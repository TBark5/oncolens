...
| 15 | Test set scored once for all tuned models; only the pre-chosen model/threshold are the headline result. | Transparency without letting test results influence any choice. |
| 16 | SHAP via exact `LinearExplainer` (interventional, training background) in log-odds units; permutation explainer fallback for non-linear models. | Exact and fast; contributions sum to the model's log-odds (checked in tests with tolerance 1e-8). |
...
| 21 | Stretch: XGBoost added as a fifth model with the same pipeline, CV, and a 16-point grid (`n_jobs=1` for determinism). | Logistic regression still wins nested CV (0.9949 vs 0.9936), so no downstream result changed. XGBoost has the lowest out-of-fold ECE (0.012); docs say so. |
| 22 | Removed `seaborn` from requirements. | It was never imported. |
| 23 | `scripts/08_summary_tables.py` rewrites the README results block in place; `tests/test_docs.py` fails if they drift. | Guarantees README numbers equal `results/`. |
| 24 | Independent review pass: fixed Brier-score ambiguity (nested vs out-of-fold), limited the README test claim to the tables, softened "between the class averages" to "most features", SHAP base value described as mean log-odds, calibration plot uses the chosen model, stale model counts, and added a "mild selection optimism" limitation. | Every doc claim must be exactly supported by `results/`. |
